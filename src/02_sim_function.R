...
#' @param sigma Numeric. Between-plot SD on the logit scale. (estimated from earlier)
...
sim_power_baci <- function(p0,
  ...
  # Outer tryCatch ensures the function always returns a numeric even if
  # something goes wrong in a parallel worker (e.g. a C-level Matrix error
  # that escapes the inner tryCatch on the glmer call).
  tryCatch({
    ...
    for (i in seq_len(n_sim)) {
      u       <- rnorm(n_plots_tot, mean = 0, sd = sigma)
    ...
    warning("sim_power_baci failed for p0=", round(p0, 3),
            ", sigma=", round(sigma, 3), ": ", conditionMessage(e))
    NA_real_
  })
}
