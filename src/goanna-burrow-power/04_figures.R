...
#   fig_encounter_rate_power.png  -- power vs days of effort, response 1,
#                                     faceted by sigma_year, coloured by
#                                     effect size, split by lambda0 variant
#   fig_species_power.png         -- power vs days of effort, response 2,
...
fig_encounter_rate <- encounter_rate_grid |>
  ...
  labs(
    x = "Field days of effort per design cell",
    y = "Simulated power",
...
tbl_singular_fits <- bind_rows(
  encounter_rate_grid |>
    select(response = lambda0_variant, days_per_cell, effect_label,
           sigma_year_label, singular_fraction) |>
    mutate(response = paste0("encounter_rate (", response, ")")),
  species_grid |>
    select(response = species_label, days_per_cell, effect_label,
           sigma_year_label, singular_fraction)
) |>
